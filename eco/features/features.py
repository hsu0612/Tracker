...
import sys
PROJ_DIR = '/home/hsu/'
sys.path.append(os.path.join(PROJ_DIR, 'pyECO'))

from eco.config import config
from eco.features import _gradient
from eco.features.model import FCNet
from eco.features.model import Discriminator
...
class CNNFeature(Feature):
    ...
    def get_features(self, img, pos, sample_sz, scales):
        ...
        for scale in scales:
            ...
            # cv2.imwrite("./test.jpg", patch[68+34:204-34, 68+34:204-34, :])
            # assert False
...
class Trainer(CNNFeature):
    def __init__(self, fname, compressed_dim):
        ...
        self._avg_pool2d = AvgPool2D()
        # input data init
        self.data_transformation = transforms.Compose([
            transforms.ToTensor(),
            ])
        self.data_type = torch.float
        self.background_model = FCNet().to(dtype = self.data_type)
        self.foreground_model = Discriminator().to(dtype = self.data_type)
        np.random.seed(999)
        ...
        self.img_count = 0
        self.lr_rate = 1.0
        self.memory = torch.zeros([3, 3, 272, 272])
        self.ema = EMA(self.foreground_model.cuda(), 0.999)
        self.ema.register()
        self.ema_b = EMA(self.background_model.cuda(), 0.999)
        self.ema_b.register()
    def init_size(self, img_sample_sz, cell_size=None):
        img_sample_sz = img_sample_sz.astype(np.int32)
        feat1_shape = np.ceil(img_sample_sz / 2)
        feat2_shape = np.ceil(img_sample_sz / 16)
        desired_sz = feat2_shape + 1 + feat2_shape % 2
        img_sample_sz = desired_sz * 16
        self.num_dim = [64, 512]
        self.sample_sz = img_sample_sz
        self.data_sz = [np.ceil(img_sample_sz / 2),
                        np.ceil(img_sample_sz / 16)]
        return img_sample_sz
    def train(self, img, pos, sample_sz, scales):
        if not isinstance(scales, list) and not isinstance(scales, np.ndarray):
            scales = [scales]
        patches = []
        for scale in scales:
            patch = self._sample_patch(img, pos, sample_sz*scale, sample_sz)
            patch = self.data_transformation(patch)
            patches.append(patch)
        patches = torch.stack(patches)
        if self.img_count == 0:
            self.memory[0] = patches
            self.memory[1] = patches
            self.memory[2] = patches
        patches = patches.to(dtype = self.data_type)
        self.memory[self.img_count%2] = patches
        # training
        # self.background_model = FCNet().to(dtype = self.data_type)
        self.background_model.train()
        self.background_model = self.background_model.cuda()
        optimizer = optim.Adam(self.background_model.parameters(), lr = 5e-4)
        ...
        for i in range(0, iter, 1):
            optimizer.zero_grad()
            p, f_1, f_2 = self.background_model(self.memory)
            pr = torch.nn.functional.interpolate(p, (patches.shape[2], patches.shape[3]), mode='bilinear', align_corners=True)
            background_diff = torch.abs(self.memory - pr)
            background_diff[:, :, int(pr.shape[2]/2) - int(pr.shape[2]/8*1):int(pr.shape[2]/2) + int(pr.shape[2]/8*1), 
            int(pr.shape[3]/2) - int(pr.shape[3]/8*1):int(pr.shape[3]/2) + int(pr.shape[3]/8*1)] = 0.0
            loss = background_diff.mean()
            if i % 100 == 0:
                print(loss)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.background_model.parameters(), 1.0)
            optimizer.step()
            self.ema_b.update()
        ...
        # check pred
        with torch.no_grad():
            pred, f_1, f_2 = self.background_model(self.memory)
        # check error map
        error_map = torch.abs(pr - self.memory)
        error_map = error_map.mean(axis = 1)
        error_map_color = error_map.detach().cpu().numpy()
        error_map_color = error_map_color*255
        error_map_color = error_map_color.astype(np.uint8)
        error_map_color = cv2.applyColorMap(error_map_color[0], cv2.COLORMAP_JET)
        cv2.imwrite("./error" + str(self.img_count) + ".jpg", error_map_color)

        # check threshold map
        with torch.no_grad():
            threshold_map = torch.nn.functional.threshold(error_map, 0.1, 0.0, inplace=False)
        threshold_map[threshold_map!=0.0] = 1.0
        threshold_map_mask_center= torch.zeros(threshold_map.shape)
        threshold_map_mask_center[:, int(pr.shape[2]/2) - int(pr.shape[2]/8*1):int(pr.shape[2]/2) + int(pr.shape[2]/8*1), 
            int(pr.shape[3]/2) - int(pr.shape[3]/8*1):int(pr.shape[3]/2) + int(pr.shape[3]/8*1)] = threshold_map[:, int(pr.shape[2]/2) - int(pr.shape[2]/8*1):int(pr.shape[2]/2) + int(pr.shape[2]/8*1), 
            int(pr.shape[3]/2) - int(pr.shape[3]/8*1):int(pr.shape[3]/2) + int(pr.shape[3]/8*1)]
        threshold_map = threshold_map_mask_center
        threshold_map_pil = torchvision.transforms.ToPILImage()(threshold_map[0].detach().cpu())
        threshold_map_pil.save("./threshold_map" + str(self.img_count) + ".jpg")
        ...
        if self.img_count % 5 == 0:
            self.lr_rate *= 0.5
        optimizer = optim.Adam(self.foreground_model.parameters(), lr = 2e-4)
        criterion_bec_loss = nn.BCELoss()
        self.foreground_model.train()
        patches_pil = torchvision.transforms.ToPILImage()(patches[0].detach().cpu())
        patches_pil.save("./input" + str(self.img_count) + ".jpg")
        self.img_count += 1
        patches = patches.cuda()
        self.foreground_model = self.foreground_model.cuda()
        for i in range(0, iter, 1):
            optimizer.zero_grad()
            p, f_1, f_2 = self.foreground_model(self.memory)
            pr = torch.nn.functional.interpolate(p, (patches.shape[2], patches.shape[3]), mode='bilinear', align_corners=True)
            # pr = pr.mean(axis=1, keepdim=True)
            # foreground_diff = torch.abs(patches - pr) * threshold_map.cuda()
            correlation_loss = criterion_bec_loss(pr, threshold_map.cuda())
            f_loss = (f_1 - f_1.mean(dim = 0, keepdim = True)).mean() + (f_2 - f_2.mean(dim = 0, keepdim = True)).mean()
            loss = correlation_loss.mean() + f_loss
            if i % 100 == 0:
                print(loss)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.foreground_model.parameters(), 1.0)
            optimizer.step()
            self.ema.update()
        ...
        with torch.no_grad():
            pred, f_1, f_2 = self.foreground_model(patches)
            pred_2 = torch.nn.functional.interpolate(pred, (patches.shape[2], patches.shape[3]), mode='bilinear', align_corners=True)
        pred_pil = torchvision.transforms.ToPILImage()(pred[0].detach().cpu())
        pred_pil.save("./img_with_foreground" + str(self.img_count) + ".jpg")
    ...
    def get_features(self, img, pos, sample_sz, scales):
        ...
        with torch.no_grad():
            pred, f_1, f_2 = self.foreground_model(patches)
        f_1 = f_1.permute(2, 3, 1, 0)
        f_2 = f_2.permute(2, 3, 1, 0)
        print(f_1.shape)
        print(f_2.shape)
        return f_1.detach().cpu().numpy(), f_2.detach().cpu().numpy()

if __name__ == '__main__':
    a = Trainer(1 ,2)
    b = Image.open("/media/hsu/data/GOT/val/GOT-10k_Val_000001/00000001.jpg")
    b = np.array(b)
    c = [377., 406.]
    d = [272., 272.]
    a.train(b, c, np.array(d), 6.82)
    a.get_features(b, c, np.array(d), 6.82)
