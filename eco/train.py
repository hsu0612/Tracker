    ...
    for i in range(num_features):
        ...
        if not config.use_gpu:
            with warnings.catch_warnings():
                ...
                if reg_pad == 0:
                    hf_out[i] += convolve(hf_conv[:, :, :], reg_filter[i][:,:,np.newaxis,np.newaxis], 'valid')
                else:
    ...
    for i in range(num_features):
        ...
        if not config.use_gpu:
            with warnings.catch_warnings():
                ...
                # do final convolution and put together result
                if reg_pad == 0:
                    hf_out1[i] += convolve(hf_conv[:, :, :], reg_filter[i][:, :, np.newaxis, np.newaxis], 'valid')
                else:
            ...
            # do final convolution and put together result
            if reg_pad == 0:
                hf_out1[i] += convolve2d(hf_conv[:, :, :], reg_filter[i][:,:,cp.newaxis,cp.newaxis], 'valid')
            else:
