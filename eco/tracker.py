...
from .features import FHogFeature, TableFeature, mround, ResNet50Feature, VGG16Feature, Trainer
...
class ECOTracker:
    ...
    def init(self, frame, bbox, total_frame=np.inf):
        ...
        for idx, feature in enumerate(features):
            if feature['fname'] == 'cn' or feature['fname'] == 'ic':
                ...
                if netname == 'resnet50':
                    ...
                    self._features.append(Trainer(**feature))
            else:
        ...
        # self._features[-1].train(frame, sample_pos, self._img_sample_sz, self._current_scale_factor)
    ...
    def update(self, frame, train=True, vis=False):
        ...
        self._pos = pos
        self._frame_num += 1
        # self._features[-1].train(frame, mround(self._pos), self._img_sample_sz, self._current_scale_factor)
        return bbox
