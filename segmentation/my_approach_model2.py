...
class AE_Segmentation2():
    ...
    def train(self, image_batch, img_without_augmentation, grid, num1, num2):
        ...
        for iter in range(0, 1001, 1):
            ...
            pred, feature_map = self.foreground_model(train_set.to("cuda:1", dtype=torch.float32))

            loss = criterion_bec_loss(pred, gt_set.to("cuda:1", dtype=torch.float32))
            loss.backward()
            optimizer_fore.step()
            if iter % 100 == 0:
                print(loss)
    ...
    #     self.foreground_model = torch.load("./exp3/checkpoint/foreground_save_" + str(num1) + "_"+ str(num2) + ".pt")
    #     self.foreground_model = self.foreground_model.to("cuda:1", dtype=torch.float32)
    #     with torch.no_grad():
    ...
    #     # error_map = torch.abs(pred - img_batch.to("cuda:1", dtype=torch.float32))
    #     # error_map = error_map.sum(axis = 1)
    #     # error_map = (error_map - error_map.min()) / (error_map.max() - error_map.min())
    ...
    #     # function.write_heat_map(mask, 0, "./threshold_background_" + str(num2) + "_")
    ...
    def inference(self, img_without_augmentation, grid, i, j):
        ...
        # pred_pil = torchvision.transforms.ToPILImage()(pred[0].detach().cpu())
        # pred_pil.save("./pred_img_with_foreground_" + str(j) + "_" + str(0) + ".jpg")
        ...
        mask_temp = np.zeros_like(mask)
        mask_temp[32:96, 32:96] = mask[32:96, 32:96]
        function.write_heat_map(mask, 0, "./threshold_background_" + str(j) + "_")
