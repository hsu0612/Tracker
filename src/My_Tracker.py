...
class FCAE_tracker():
    ...
    def tracker_inference_for_eval(self, img, video_num, flag):
        ...
        self.x, self.y, self.w, self.h = function.get_obj_x_y_w_h(threshold_map, threshold_map_seg, self.x, self.y, self.w, self.h, img, self.device, self.data_type, self.model_discriminator)
    ...
    def tracker_update(self, video_num):
        ...
        self.model_discriminator.train()
        # train data init
        self.training_set = torch.cat((self.memory[self.count_image-4], self.memory[self.count_image-1]), 0)
    ...
    def tracker_inference_for_consistancy(self, video_num):
    ...
    def tracker_consistancy(self, img, video_count, number_of_frame, video_num):
        # input data init
        data_transformation = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor(),
            ])
